...
import AddMyWork from "./AddMyWork";
import Cookies from "js-cookie";
import EditMyWork from "./EditMywork";
import RemoveMyWork from "./RemoveWork";
...
const MyWork = () => {
  ...
  const getWorkData = async () => {
    try {
      ...
      const { data } = await axios.get(
        `${url}/a_admin_panel/api/jobs_list_views/`, config
      );
      setWork(data?.data?.results);
  ...
  const handleEdit = (id) => {
    setEditId(id);
    setModals({ ...modals, edit: true, delete: false, add: false });
  };
  const handleRemove = (id) => {
    setEditId(id);
    setModals({ ...modals, delete: true, edit: false, add: false });
  };
    ...
    <AdminLayout>
      <div className="min-h-[100vh] py-16 ">
        <div className=" w-[90%] lg:w-[98%] mx-auto mb-12 ml-[6%] sm:ml-[1%] items-center mt-2 flex justify-between">
          <p className="text-white font-semibold text-[20px]">Работа</p>
          <IconButton
            onClick={() =>
              setModals({ ...modals, add: true, delete: false, edit: false })
            }
            sx={{ cursor: "pointer" }}
          >
        ...
        <div class="rounded-sm  border-stroke bg-[#24303f] w-[98%] mx-auto px-5 pt-6 pb-2.5 shadow-default sm:px-7.5 xl:pb-1">
          <div class="max-w-full overflow-x-auto">
            <table class="w-full table-auto">
              ...
              {modals.add && (
                <AddMyWork getData={getWorkData} setIsOpen={setModals} />
              )}

              {modals.edit && (
                <EditMyWork getData={getWorkData} id={editId} setIsOpen={setModals} />
              )}

             
              {modals.delete && (
                <RemoveMyWork getData={getWorkData} setIsRemove={setModals} id={editId}/>
              )} 
                ...
                {work?.map((c, index) => (
                  <tr>
                    ...
                    <td class="border-b border-[#eee] py-5 pl-2 dark:border-strokedark xl:pl-11">
                      <div className="h-[150px] w-[150px] sm:w-[200px] ">
                      <img src={url+c?.img} alt="" className="w-full h-full rounded-xl" />
                      </div>
                    </td>

                    <td class="border-b border-[#eee] py-5 pl-[4%] dark:border-strokedark xl:pl-11">
