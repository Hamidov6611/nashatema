...
const EditCategory = ({ setIsOpen, getData, id }) => {
  ...
  const getOneSubCategory = async () => {
    try {
      const { data } = await axios.get(
        `${url}/a_admin_panel/api/category_crud_views/${id}/`
      );
      setData(data[0]);
      console.log(data[0]?.name);
      setName(data[0]?.name);
      setIdCategory(data[0]?.id_category?.id);
      console.log([data[0]])
    } catch (error) {
      console.log(error);
    }
  ...
  const EditCategory = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`${url}/a_admin_panel/api/category_crud_views/${id}/`, {
        name,
        id_category,
      });
      getData();
    ...
    <div className="fixed top-0 left-0 w-full h-[100vh] bg-modal flex items-center justify-center z-50">
      ...
      >
        ...
        <div className="flex w-full my-8 flex-col gap-y-4">
          ...
          >
            <option value={data?.id_category?.id} className="text-slate-400 cursor-auto">{data?.id_category?.name}</option>
            {cat?.map((c, i) => (
