...
const AddCategory = ({ setIsOpen, getData }) => {
  ...
  const AddCategory = async (e) => {
    e.preventDefault()
    try {
        await axios.post(`${url}/a_admin_panel/api/category_list_views/`, {name, id_category})
        getData()
        setIsOpen(false)
    } catch (error) {
        console.log(error)
  ...
  return (
    <div className="fixed top-0 left-0 w-full h-[100vh] bg-modal flex items-center justify-center z-50">
      <form onSubmit={AddCategory} className="rounded-md w-[90%] md:w-[50%] p-4 lg:w-[30%] h-[270px] bg-white">
