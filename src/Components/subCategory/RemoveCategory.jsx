...
const RemoveCategory = ({ setIsRemove, getData, id }) => {
  const closeHandler = () => setIsRemove(false);
  const RemoveCategory = async (e) => {
    e.preventDefault()
    try {
        await axios.delete(`${url}/a_admin_panel/api/category_crud_views/${id}/`)
        getData()
        setIsRemove(false)
    } catch (error) {
        console.log(error)
  ...
  return (
    <div className="fixed top-0 left-0 w-full h-[100vh] bg-modal flex items-center justify-center z-50">
      <form onSubmit={RemoveCategory} className="rounded-md w-[90%] md:w-[50%] p-4 lg:w-[30%] h-[140px] bg-white">
        <div className="w-full flex items-center justify-between text-[#343434] font-semibold text-[16px]">
          <p>Удалить подкатегория</p>
          <svg
