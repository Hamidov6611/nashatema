...
const EditCategory = ({ setIsOpen, getData, id }) => {
  ...
  const EditCategory = async (e) => {
    e.preventDefault()
    try {
        await axios.put(`${url}/a_admin_panel/api/main_category_crud_views/${id}/`, {name})
        getData()
        setIsOpen(false)
    } catch (error) {
        console.log(error)
  ...
  const getPrevData = async () => {
    try {
        const {data} = await axios.get(`${url}/a_admin_panel/api/main_category_crud_views/${id}/`)
        console.log(data)
        setOldData(data[0])
        setName(data[0]?.name)
    } catch (error) {
  ...
  return (
    <div className="fixed top-0 left-0 w-full h-[100vh] bg-modal flex items-center justify-center z-50">
      <form onSubmit={EditCategory} className="rounded-md w-[90%] md:w-[50%] p-4 lg:w-[30%] h-[220px] bg-white">
        ...
        <button type="submit" className="w-full bg-[#343434] text-white font-semibold py-2 rounded-lg">
        Изменить
        </button>
      </form>
    </div>
